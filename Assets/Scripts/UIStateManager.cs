...
public class UIStateManager : MonoBehaviour {
    ...
    void Update() {
        ...
        if (Physics.Raycast(ray, out groundHit, 1000, groundLayerMask) &&
            groundHit.collider.gameObject.CompareTag("terrain")) {
            mousePosition = groundHit.point;
        }

        RaycastHit uiHit;
        bool isUiHit = Physics.Raycast(ray, out uiHit, 1000, uiLayerMask) &&
                       uiHit.collider.gameObject.CompareTag("SplineHandle");
